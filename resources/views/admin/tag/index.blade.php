...
<script>
    $(document).ready(function() {
        ...
        $('#tags-table').DataTable({
            ...
            serverSide:true,
            ajax: {
            url: "{{ route('tags.get-data') }}",
                error: function(xhr, textStatus, errorThrown) {
                    $('#tags-table').DataTable().clear().draw();
        ...
        $("div.toolbar").html('<button class="ms-2 btn btn-primary tags-add" type="button" data-bs-target="#tabs-add-tag">'+
                                '<span>Create Tag</span>'+
                                '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>'+
                            '</button>');

        // Event create by Modal
        $(document).on('click', '.tags-add', function() {
            var getTarget = $(this).data('bs-target');

            $.get("{{ route('tags.modal-add') }}", function(data) {
        ...
        // Event Edit by Modal
        $(document).on('click', '.tags-edit-table', function() {
            var userId = $(this).data('bs-target');
            var parts = userId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('tags/modal-edit') }}/" + id, function(data) {
                $('#modalContainer').html(data);
                $(`${userId}`).modal('show');
            });
        });
        ...
        $(document).on('click', '.tags-delete-table', function() {
            var userId = $(this).data('bs-target');
            var parts = userId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('tags/modal-delete') }}/" + id, function(data) {
                $('#modalContainer').html(data);
                $(`${userId}`).modal('show');
            });
        });
