...
<div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
    ...
    <div class="widget-content widget-content-area br-8">
        <table id="materials-table" class="table dt-table-hover" style="width:100%">
            <thead>
                <tr>
                    <th width="7%">No</th>
                    <th>Code</th>
...
<script>
    $(document).ready(function() {
        ...
        $('#materials-table').DataTable({
            ...
            serverSide:true,
            ajax: {
            url: "{{ route('materials.get-data') }}",
                error: function(xhr, textStatus, errorThrown) {
                    $('#materials-table').DataTable().clear().draw(); // Bersihkan tabel
        ...
        $("div.toolbar").html('<button class="ms-2 btn btn-primary materials-add" type="button" data-bs-target="#tabs-add-material">'+
                                '<span>Create Material</span>'+
                                '<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-plus"><line x1="12" y1="5" x2="12" y2="19"></line><line x1="5" y1="12" x2="19" y2="12"></line></svg>'+
                            '</button>');

        // Event create by Modal
        $(document).on('click', '.materials-add', function() {
            var getTarget = $(this).data('bs-target');

            $.get("{{ route('materials.modal-add') }}", function(data) {
        ...
        // Event Edit by Modal
        $(document).on('click', '.materials-edit-table', function() {
            var userId = $(this).data('bs-target');
            var parts = userId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('materials/modal-edit') }}/" + id, function(data) {
                $('#modalContainer').html(data);
                $(`${userId}`).modal('show');
            });
        });
        ...
        $(document).on('click', '.materials-delete-table', function() {
            var materialId = $(this).data('bs-target');
            var parts = materialId.split("-");
            var id = parseInt(parts[1]);

            $.get("{{ url('materials/modal-delete') }}/" + id, function(data) {
                $('#modalContainer').html(data);
